  ...
  template: `
    <div class="mx-auto max-w-2xl">
      ...
      @for (deadline of deadlines(); track deadline.id) {
        <div
          class="mb-3 flex items-start gap-3 rounded-lg border bg-white p-4 shadow-sm"
        ...
        >
          ...
          <div class="flex-1">
            <h3 class="font-semibold" [class.line-through]="deadline.completed" [class.text-gray-400]="deadline.completed">
              {{ deadline.title }}
            </h3>
            <p class="text-sm text-gray-500">{{ deadline.description }}</p>
            <div class="mt-2 flex items-center gap-3">
              <span class="rounded-full px-2 py-0.5 text-xs font-medium"
                [class]="'bg-' + (deadline.type === 'tma' ? 'blue' : deadline.type === 'exam' ? 'red' : deadline.type === 'practical' ? 'purple' : 'green') + '-100 text-' + (deadline.type === 'tma' ? 'blue' : deadline.type === 'exam' ? 'red' : deadline.type === 'practical' ? 'purple' : 'green') + '-700'">
                {{ deadline.type }}
              </span>
              <span class="text-xs text-gray-400">{{ deadline.date | date: 'MMM d, y' }}</span>
              @if (!deadline.completed) {
                <span class="text-sm font-semibold"
                  [class]="isOverdue(deadline) ? 'text-error' : isUrgent(deadline) ? 'text-warning-dark' : 'text-primary'">
      ...
      @if (showForm()) {
        <div class="mt-4 rounded-lg border bg-white p-4 shadow-sm">
          ...
          <div class="flex flex-col gap-3">
            ...
            <textarea [(ngModel)]="newDesc" rows="2" placeholder="Description"
              class="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"></textarea>
            <div class="flex justify-end gap-2">
              <button class="rounded-lg px-4 py-2 text-sm text-gray-600 hover:bg-gray-100" (click)="showForm.set(false)">Cancel</button>
        ...
        <button
          class="fixed bottom-6 right-6 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-white shadow-lg hover:bg-primary-dark"
...
export class DeadlinesComponent implements OnInit {
  ...
  ngOnInit(): void { this.deadlineService.deadlines$.subscribe((d) => this.deadlines.set(d)); }
  ...
  async addDeadline(): Promise<void> {
    ...
    this.newTitle = ''; this.newDate = ''; this.newDesc = ''; this.showForm.set(false);
  }
}
